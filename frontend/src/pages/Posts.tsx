...
function Posts() {
  ...
  const searchPosts = async (term: string, userId: string) => {
    try {
      const searchParams = new URLSearchParams();
      if (term) searchParams.append('term', term);
      if (userId && userId !== 'all') searchParams.append('userId', userId);

      const res = await get(`${APIURLS.POSTS}/search?${searchParams.toString()}`);
      setPosts(res.data);
    } catch (err) {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      searchPosts(searchTerm, selectedUserId);
    }, 300);
